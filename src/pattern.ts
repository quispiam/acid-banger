...
export type NineOhPatterns = {
    patterns: number[][];
    sampleVariants: number[];  // per-track: 0 = synth sample, 1 = 909 sample
}

export function NineOhGen() {
    function createPatterns(full: boolean = false): NineOhPatterns {
        ...
        const kickMode: string = choose(["electro", "fourfloor", "dnb", "halftime"]);
        const hatMode: string  = choose(["offbeats", "closed", "rolling", full ? "offbeats" : "none"]);
        const snareMode: string = choose(["backbeat", "skip", "dnb", full ? "backbeat" : "none"]);
        ...
        if (hatMode == "offbeats") {
            ...
            for (let i = 0; i < 16; i++) {
                chPattern[i] = 0.2 + Math.random() * 0.3;
                if (i % 8 === 0) chPattern[i] = 0.5;   // accent on beats
