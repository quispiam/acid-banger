...
function ClockControls(clock) {
    ...
    clock.randomizeBpm.subscribe(v => {
        if (v)
            rndBtn.classList.add("waiting");
        else
            rndBtn.classList.remove("waiting");
    });
    ...
    function bpmRangeField(param, labelText) {
        const wrapper = document.createElement("div");
    ...
    rangeContainer.append(bpmRangeField(clock.bpmMin, "Min"), bpmRangeField(clock.bpmMax, "Max"));
...
function RandomisationControls(autoPilot) {
    const r = autoPilot.randomisation;
    function rndField(param, isFloat = false) {
        const wrapper = document.createElement("div");
    ...
    function rndSection(title, ...fields) {
...
export function UI(state, autoPilot, analyser, midi) {
    ...
    ui.id = "ui";
    const otherControls = controls(AutopilotControls(autoPilot), NoteGen(state.gen), DelayControls(state.delay), ClockControls(state.clock), controlGroup(label("Volume"), DialSet([state.masterVolume], "horizontal")), controlGroup(label("Meter"), group(AudioMeter(analyser)), "meter"));
